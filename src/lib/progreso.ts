 ...
 */
export async function registrarLeccionCompletada(indiceLeccion: number, puntosGanados: number) {
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) throw new Error('No hay usuario con sesión activa')
 
  const { error: errorSesion } = await supabase.from('sesiones_estudio').insert({
    usuario_id: user.id,
    tecnica_id: `leccion-${indiceLeccion + 1}`,
  ...
  const nuevaLeccionActual = indiceLeccion === leccionActualPrevia ? leccionActualPrevia + 1 : leccionActualPrevia
  ...
  const { error: errorProgreso } = await supabase.from('progreso_usuario').upsert({
    ...
    puntos_totales: nuevosPuntos,
    nivel: nuevoNivel,
    leccion_actual: nuevaLeccionActual,
    actualizado_en: new Date().toISOString(),
  })
  if (errorProgreso) throw errorProgreso
}
